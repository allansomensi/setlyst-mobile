...
export function ChordProRenderer({
  ...
  const parseSectionInfo = (text?: string, defaultKey?: ToolbarKey) => {
    if (!text) {
      return {
        key: defaultKey || null,
        label: defaultKey ? t(`lyrics.toolbar.${defaultKey}`) : "",
      };
    }
    ...
    if (defaultKey && /^\d+$/.test(trimmed)) {
      return {
        key: defaultKey,
        label: `${t(`lyrics.toolbar.${defaultKey}`)} ${trimmed}`,
      };
    }
    ...
    for (const key of sortedKeys) {
      ...
      if (match) {
        ...
        const translated = t(`lyrics.toolbar.${mappedKey}`);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
      ...
      if (dir) {
        ...
        if (directive === "eov" || directive === "end_of_verse") continue;
        ...
        if (directive === "eob" || directive === "end_of_bridge") continue;
    ...
    elements.push(
      <div
        key={i}
        className={cx(lineWrapperClass, "text-foreground font-medium")}
      >
  ...
  return (
    <div
      className={cx(fontClass, className, "max-w-3xl")}
      style={{ fontSize: `${fontSize}rem` }}
    >
      {elements}
    </div>
